...
export default function ProgramCard({
  ...
  // İkinci üretim yolu (isteğe bağlı): 4 haftalık v3 pipeline (uygulamadaki Fit AI Koç ile aynı)
  onGenerateV3, generatingV3, generateLabel,
  ...
  const busy = !!(generating || generatingV3);
        ...
        <div className="flex items-center gap-2">
          {onGenerate && (
            <button
              onClick={onGenerate}
              disabled={busy}
              title={onGenerateV3 ? "v2: tek haftalık program, her hafta tekrar eder" : undefined}
            ...
            >
              {generating ? (
                ...
                <>
                  ...
                  <span>{generateLabel || "AI ile Üret"}</span>
                </>
              )}
            </button>
          )}
          {onGenerateV3 && (
            <button
              ...
              disabled={busy}
              title="v3: uygulamadaki Fit AI Koç ile aynı kuralcı pipeline — 4 haftalık ilerleyici program (1-2 dk)"
              className="rounded-xl bg-gradient-to-r from-[#3E9E8E] to-[#2B7B6E] px-3 py-2 text-sm font-medium text-white hover:from-[#2B7B6E] hover:to-[#215F54] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
            >
              {generatingV3 ? (
                <>
                  <Spinner />
                  <span>4 hafta üretiliyor...</span>
                </>
              ) : (
                <>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <span>4 Haftalık Program (v3)</span>
                </>
              )}
            </button>
          )}
